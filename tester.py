...
def execute(config, test_type, input_file=None) -> subprocess.CompletedProcess[str]:
    ...
    match test_type:
        case TestType.PYTHON:
            kmeans_path = project_root / "kmeans.py"
            args = ["python3", str(kmeans_path), config["k"]]
        case TestType.C:
            kmeans_binary = project_root / "kmeans"
            args = [str(kmeans_binary), config["k"]]
...
def main():
    ...
    for test_type, applicable in (
        ...
        result = execute(_config, test_type, dummy_input)
        dummy_input.seek(0)
